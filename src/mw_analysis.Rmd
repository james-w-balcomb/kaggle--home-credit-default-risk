...
for (i in 1:length(class.values)){
  ...
  if (cv == "numeric") {
    ...
    log.vals = log(train.X[, cn] + 1)
    train.X[, new.name] <- log.vals
  }
...
train.X <- train.X[, -121] # removes TARGET column
class.values <- sapply(train.X, class) # update class.values with new columns
ints <- as.vector(class.values == "integer")
nums <- as.vector(class.values == "numeric")
chars <- as.vector(class.values == "character")
...
for (i in 1:length(class.values)) {
  ...
  # identify numerics and find their log
  if (cv != "numeric") {
    mi = mutinformation(y, train.X[, cn])
