...
tr <- tr[, keep.all]
```
### Add additional features to train set:
...
housing$TARGET <- as.numeric(tr$TARGET) - 1
...
tapply(housing$TARGET, flags, mean)
tr$FLAG_DOCS_PROVIDED <- flags

names(tr)

y <- housing$TARGET
own_both <- tr$FLAG_OWN_CAR + tr$FLAG_OWN_REALTY
tapply(y, tr$CODE_GENDER, mean)

table(tr$CODE_GENDER)
